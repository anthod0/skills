...
test('submission scope tracks additions, deletions, and forbidden edits, without constraining test names', () => {
  assert.deepEqual(changesBetween(original, submitted).map(({ path }) => path), ['tests/new.test.mjs', 'tests/old.test.mjs']);
  assert.equal(checkScope(changesBetween(original, submitted), ['tests/**']).ok, true);
  for (const files of [{ ...submitted, 'src/a.mjs': 'changed' }, { 'tests/new.test.mjs': 'checks' }, { ...submitted, 'tests-escape/file': 'unexpected' }]) {
  ...
  assert.deepEqual(jobs.map((job) => job.expected), ['pass', 'assertion-failure', 'pass']);
  ...
  assert.throws(() => createSubmissionPlan({ ...submitted, 'src/a.mjs': 'different' }, variants, ['node', '--test']), /exactly once/);
});
...
test('agent regressions accept renamed assertion candidates, not crashes; calibration keeps designated names', () => {
  ...
  const run = (code) => ({ code: 1, stdout: [
    { type: 'test:fail', name: 'new consolidated behavior', subtype: 'test', error: { failureType: 'testCodeFailure', cause: { code } } },
    { type: 'test:summary', counts: { tests: 1, failed: 1, passed: 0, skipped: 0, todo: 0, cancelled: 0 } },
...
test('nested assertions permit propagated parent failures but never mask ordinary or hook errors', () => {
  const job = createSubmissionPlan(submitted, variants, ['node', '--test'])[1];
  const assertion = { type: 'test:fail', name: 'nested behavior', subtype: 'test', error: { failureType: 'testCodeFailure', cause: { code: 'ERR_ASSERTION' } } };
  const parent = { type: 'test:fail', name: 'parent', subtype: 'test', error: { failureType: 'subtestsFailed' } };
  const suite = { ...parent, name: 'suite', subtype: 'suite' };
  const run = (records, failed) => ({ code: 1, stdout: [
    ...records, { type: 'test:summary', counts: { tests: failed, failed, passed: 0, skipped: 0, todo: 0, cancelled: 0 } },
  ].map((event) => JSON.stringify(event)).join('\n') });
  ...
  const thrown = { ...assertion, error: { failureType: 'testCodeFailure', cause: { message: 'ordinary exception' } } };
  assert.equal(assess(job, run([assertion, thrown, parent], 3)).outcome, 'invalid-run');
  const hookFailure = { ...suite, error: { failureType: 'hookFailed', cause: { code: 'ERR_ASSERTION' } } };
...
test('workspace export rejects escapes, non-text files, links, and oversized files without following them', async () => {
  ...
  try {
    ...
    for (const files of [{ '../escape': 'x' }, { '/absolute': 'x' }, { '.git/config': 'x' }, { a: '\0' }, { a: 'x'.repeat(1024 * 1024 + 1) }]) {
    ...
    assert.deepEqual(Object.keys(await readTree(testRoot, { ignoreGit: true })), ['tests/example.mjs']);
  } finally {
    const belowTemporary = relative(temporaryBase, testRoot);
    assert.ok(belowTemporary && !belowTemporary.startsWith('..') && !belowTemporary.startsWith(sep));
