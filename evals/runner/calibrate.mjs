...
async function main() {
  ...
  if (flag === "--check") {
    console.log(
      `${caseId}: ${Object.keys(files).length} files, ${variants.length} variants, ${jobs.length} isolated runs planned. No fixture executed.`,
    );
    return;
  }
  ...
  try {
    ...
    for (const job of jobs) {
      ...
      const execution = await runInContainer(
        image,
      ...
      report.jobs.push({
        ...
        cleanupFailed: execution.cleanupFailed ?? false,
        ...verdict,
      });
      await save();
