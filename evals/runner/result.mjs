export function assess(job, execution) {
  ...
  if (summaries.length !== 1 || !(summaries[0].counts?.tests > 0))
    return invalid("No completed test suite");
  const counts = summaries[0].counts;
  if (counts.skipped || counts.todo || counts.cancelled)
    return invalid("Skipped, TODO, or cancelled tests");
  ...
  if (!tests.length || records.some((event) => event.skip || event.todo))
    return invalid("Missing or skipped tests");
  const failures = tests.filter((event) => event.type === "test:fail");
  if (failures.length !== counts.failed || tests.length !== counts.tests)
    return invalid("Inconsistent test report");
  const assertions = [];
  for (const event of records.filter((record) => record.type === "test:fail")) {
    ...
    if (error?.failureType !== "testCodeFailure")
      return invalid("Failure was not an assertion (crash, import, cancellation, or timeout)");
    while (error && error.code !== "ERR_ASSERTION") error = error.cause;
    if (!error)
      return invalid("Failure was not an assertion (crash, import, cancellation, or timeout)");
    assertions.push(event.name);
  }
  if (failures.length && !assertions.length)
    return invalid("No assertion evidence for propagated failures");
  const outcome = failures.length ? "assertion-failure" : "pass";
  if ((execution.code === 0) !== (outcome === "pass"))
    return invalid("Exit status disagrees with test report");
  const failedTests = failures.map((event) => event.name);
  if (outcome !== job.expected)
    return { ok: false, outcome, failedTests, reason: `Expected ${job.expected}` };
  if (
    outcome === "assertion-failure" &&
    job.detects !== undefined &&
    !job.detects.some((name) => assertions.includes(name))
  ) {
    return {
      ok: false,
      outcome,
      failedTests,
      reason: "No designated assertion detected this variant",
    };
  }
