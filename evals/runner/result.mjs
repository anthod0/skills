export function assess(job, execution) {
  ...
  const tests = events.filter((event) => ['test:pass', 'test:fail'].includes(event.type) && event.subtype !== 'suite');
  if (!tests.length || tests.some((event) => event.skip || event.todo)) return invalid('Missing or skipped tests');
  ...
  if (failures.some((event) => {
    let error = event.error;
    if (error?.failureType !== 'testCodeFailure') return true;
    while (error) {
  ...
  if (outcome === 'assertion-failure' && !job.detects.some((name) => failedTests.includes(name))) {
