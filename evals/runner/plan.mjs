...
export function applyEdits(files, edits) {
  ...
  for (const edit of edits) {
    checkPath(edit.file);
    if (!Object.hasOwn(files, edit.file) || typeof edit.before !== 'string'
        || !edit.before || typeof edit.after !== 'string') throw new Error('Invalid edit');
  ...
  for (const [file, ranges] of grouped) {
    ...
    for (const range of ranges.reverse()) {
      result[file] = result[file].slice(0, range.start) + range.after + result[file].slice(range.end);
    }
  }
  return result;
}
...
export function createPlan(files, variants, reference, command) {
  ...
  for (const variant of variants) {
    if (typeof variant.id !== 'string' || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(variant.id) || ids.has(variant.id)
        || !['regression', 'refactor'].includes(variant.kind)
        || !variant.file?.startsWith('src/') || !Array.isArray(variant.detects)
        || !variant.detects.length || variant.detects.some((name) => typeof name !== 'string' || !name)) {
      throw new Error(`Invalid variant: ${variant.id}`);
    }
    ids.add(variant.id);
  }
  const jobs = [];
  for (const [version, sources] of [['original', files], ['reference', cleaned]]) {
    jobs.push({ id: `${version}--baseline`, version, expected: 'pass', detects: [], files: { ...sources } });
    for (const variant of variants) {
      const expected = version === 'reference' && variant.kind === 'refactor' ? 'pass' : 'assertion-failure';
      jobs.push({
        id: `${version}--${variant.id}`, version, variant: variant.id, kind: variant.kind,
        expected, detects: expected === 'pass' ? [] : variant.detects,
