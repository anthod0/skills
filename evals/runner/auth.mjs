...
export function selectAuth(text, provider) {
  ...
  try {
    ...
    throw new Error("Cannot parse authentication file");
  }
  if (!auth || !Object.hasOwn(auth, provider))
    throw new Error("No stored authentication for selected provider");
...
export async function readAuth(provider, authFile) {
  const path =
    authFile ?? join(process.env.PI_CODING_AGENT_DIR ?? join(homedir(), ".pi/agent"), "auth.json");
  let text;
  try {
    text = await readFile(path, "utf8");
  } catch {
    throw new Error("Cannot read authentication file");
  }
  return selectAuth(text, provider);
}

export function redact(text, ...authStates) {
  const secrets = authStates.flatMap((auth) =>
    Object.values(auth).flatMap((credential) =>
      Object.entries(credential)
        .filter(([key, value]) => key !== "type" && typeof value === "string" && value.length > 0)
        .map(([, value]) => value),
    ),
  );
  for (const secret of [...new Set(secrets)].sort((a, b) => b.length - a.length)) {
    ...
    try {
      ...
      /* Lone surrogates have no URI encoding. */
    }
