...
export function assessPiSmoke(execution, selection) {
  let output;
  try { output = parseAgentOutput(execution); } catch (error) { return { ok: false, reason: error.message }; }
  const check = assessAgent(output, selection);
  if (!check.ok) return check;
  if (check.finalResponse !== 'PI_SMOKE_OK') return { ok: false, reason: 'Missing final smoke acknowledgement' };
  ...
  return { ok: true, piVersion, nodeVersion, uid, cwd, authMode, tools: ['read', 'write', 'edit', 'bash'], assistantMessages: check.assistantMessages };
}
