...
test('copies only selected stored authentication, rejects key references, and redacts old and refreshed secrets', () => {
  ...
  assert.throws(() => selectAuth('{ synthetic-secret', 'example'), (error) => error instanceof Error && !error.message.includes('synthetic-secret'));
  ...
  const output = redact('synthetic-access synthetic-refresh rotated-token harmless', { example: credential }, { example: { ...credential, access: 'rotated-token' } });
  assert.equal(output, '[REDACTED] [REDACTED] [REDACTED] harmless');
  for (const key of ['synthetic\\key"', 'synthetic\ud800']) {
    const redactedJson = redact(JSON.stringify({ text: key }), { example: { type: 'api_key', key } });
...
function packet() {
  return {
    runtime: { uid: 1000, cwd: '/workspace', authMode: 0o600, provider: 'example', piVersion: '0.85.1', nodeVersion: 'v22.19.0' },
    execution: { code: 0 }, files: { 'probe.txt': 'AFTER\n' },
    events: [
      ...
      { type: 'message_end', message: { role: 'assistant', ...selection, stopReason: 'stop', content: [{ type: 'text', text: 'PI_SMOKE_OK' }], usage: { input: 10, output: 2, cacheRead: 3, cacheWrite: 0, totalTokens: 15, cost: { total: 0.01 } } } },
      { type: 'agent_end' },
    ],
  };
}
...
test('requires completion from expected model, complete tool traces, and verified container state', () => {
  ...
  for (const mutate of [
    (output) => { output.runtime.uid = 0; },
    (output) => { output.runtime.authMode = 0o644; },
    (output) => { output.runtime.cwd = '/host'; },
    (output) => { output.events[1].isError = true; },
    (output) => { output.events.splice(1, 1); },
    (output) => { output.events[1].toolCallId = 'unmatched'; },
    (output) => { output.events[1].isError = 'false'; },
    (output) => { output.events[8].message.model = 'other-model'; },
    (output) => { output.events[8].message.stopReason = 'error'; },
    (output) => { output.events[8].message.content = {}; },
    (output) => { output.events.pop(); },
    (output) => { output.files['probe.txt'] = 'BEFORE\n'; },
    (output) => { output.execution.problem = 'ETIMEDOUT'; },
    (output) => { output.artifactError = 'Rejected link'; output.files = null; },
  ]) {
    const output = packet();
    mutate(output);
...
test('Pi profiles send credentials only on stdin, bound the container lifetime, and remove their own container', async () => {
  for (const budgetSeconds of [180, 600]) {
    ...
    for (const flag of ['--network=bridge', '--read-only', '--user=1000:1000', '--cap-drop=ALL', '--security-opt=no-new-privileges']) assert.ok(args.includes(flag));
    assert.ok(args.includes('--tmpfs=/run/pi-agent:rw,nosuid,nodev,noexec,uid=1000,gid=1000,mode=0700,size=16m'));
    assert.deepEqual(args.slice(-4), ['sha256:test', `${budgetSeconds}s`, 'node', '/harness/pi-entry.mjs']);
