...
const files = {
  "src/example.mjs": "const answer = 1;",
  "tests/example.test.mjs": "behavior\ncopy\n",
};
...
test("builds independent original and reference matrices without mutating input", () => {
  ...
  assert.deepEqual(
    jobs.map((job) => [job.id, job.expected]),
    [
      ["original--baseline", "pass"],
      ["original--wrong-answer", "assertion-failure"],
      ["original--renamed-local", "assertion-failure"],
      ["reference--baseline", "pass"],
      ["reference--wrong-answer", "assertion-failure"],
      ["reference--renamed-local", "pass"],
    ],
  );
...
test("refuses path escapes, production cleanup, test mutants, and arbitrary Node flags", () => {
  ...
  assert.throws(
    () =>
      createPlan(
        files,
        variants,
        [{ file: "src/example.mjs", before: "1", after: "2" }],
        ["node", "--test"],
      ),
    /only edit tests/,
  );
  assert.throws(
    () =>
      createPlan(files, [{ ...variants[0], file: "tests/example.test.mjs" }], reference, [
        "node",
        "--test",
      ]),
    /Invalid variant/,
  ...
  assert.throws(
    () => createPlan(files, variants, reference, ["node", "--test", "--import=malicious"]),
    /Unsafe|Test arguments/,
  );
  assert.throws(
...
test("container runs use isolation and remove only their own container after timeout", async () => {
  ...
  assert.equal(
    run.args.some((arg) => /^(--volume|--mount|--privileged|-v)$/.test(arg)),
    false,
  );
...
test("accepts passes and designated assertion failures, but not undetected mutations", () => {
  ...
  assert.equal(
    assess({ expected: "pass", detects: [] }, execution({ failure: assertionError })).ok,
    false,
  );
  assert.equal(
    assess(failureJob, execution({ failure: assertionError, name: "unrelated" })).ok,
    false,
  );
});
...
test("does not credit import errors, thrown exceptions, cancellation, or timeouts", () => {
  ...
  assert.equal(
    assess(failureJob, execution({ failure: assertionError, code: 137 })).outcome,
    "invalid-run",
  );
});
...
test("rejects empty, incomplete, malformed, skipped, and inconsistent reports", () => {
  for (const stdout of [
    ...
    "42",
    "{}",
    JSON.stringify({ type: "test:pass", name: "behavior" }),
  ]) {
  ...
  for (const counts of [
    ...
    assert.equal(
      assess(failureJob, execution({ failure: assertionError, counts })).outcome,
      "invalid-run",
    );
  }
  assert.equal(
    assess(failureJob, execution({ failure: assertionError, code: 0 })).outcome,
    "invalid-run",
  );
});
