...
Cover adding the first host when no config exists, listing and updating an existing host, default config location selection versus `--config`, and failure cases such as invalid edits and linked config files. Check both command results and the resulting file contents, including unrelated entries and existing permissions.
