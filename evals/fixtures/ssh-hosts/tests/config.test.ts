...
test('adds a host before wildcard defaults so its options take precedence', () => {
  ...
  const updated = updateConfig(source, {
    kind: 'add', alias: 'devbox', options: { host: '10.8.0.14', user: 'deploy', port: '2222' },
  });
  assert.deepEqual(listHosts(updated).find((host) => host.alias === 'devbox'), {
    alias: 'devbox', host: '10.8.0.14', user: 'deploy', port: '2222',
  });
...
test('rename and removal leave other hosts and their comments alone', () => {
  ...
  assert.deepEqual(listHosts(renamed).map((host) => host.alias), ['storage', 'backup']);
...
test('rejects collisions and ambiguous targets rather than choosing a block', () => {
  assert.throws(() => updateConfig(homeNetwork, {
    kind: 'add', alias: 'NAS', options: { host: '10.0.0.1' },
  }));
  for (const source of [
    'Host nas storage\n  User sam\n',
    'Host nas\n  User sam\nHost nas\n  Port 2222\n',
...
test('rejects out-of-range ports and values that could insert extra directives', () => {
  ...
  for (const port of ['1', '65535']) {
    assert.equal(listHosts(updateConfig(homeNetwork, {
      kind: 'set', alias: 'nas', options: { port },
    }))[0].port, port);
  }
});
