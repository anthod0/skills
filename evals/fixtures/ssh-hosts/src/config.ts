...
function document(text: string) {
  ...
  rows.forEach((row, index) => {
    ...
    sections.push({
      ...
      names: row.key === "host" ? row.value.split(/\s+/) : [],
    });
  });
...
function validateAlias(alias: string) {
  if (!plainAlias.test(alias))
    throw new Error(
      "Use a single host alias containing letters, digits, dots, underscores or hyphens.",
    );
}
...
function optionLines(options: HostOptions) {
  return Object.entries(options).map(([option, value]) => {
    ...
    if (option === "port") {
      ...
      if (/[\r\n"\\\0]/.test(value))
        throw new Error("Identity path contains unsupported characters.");
...
export function updateConfig(text: string, change: Change): string {
  ...
  if (rows.some((row) => row.invalid))
    throw new Error("Cannot edit this config: unrecognized directive syntax.");
