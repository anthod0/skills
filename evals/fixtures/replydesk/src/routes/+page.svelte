...
<svelte:head>
  ...
  <meta
    name="description"
    content="Turn a customer message and the facts you know into a thoughtful reply draft."
  />
</svelte:head>
  ...
  <main>
    ...
    <div class="editor-grid">
      <section class="panel" aria-labelledby="request-heading">
            ...
            return async ({ result, update }) => {
              try {
                if (result.type === "error") {
                  transportError =
                    "The request didn’t finish. Your notes are still here — try again.";
                } else {
        ...
        >
          <fieldset class="input-fields" disabled={pending}>
            <div class="field">
              ...
              <textarea
                ...
                aria-describedby={form?.errors.message ? "message-error" : undefined}></textarea>
            ...
            <div class="field">
              ...
              <p class="hint" id="context-hint">
                Confirmed facts, what you can offer, and any details to avoid sharing.
              </p>
              <textarea
                id="context"
                name="context"
                ...
                aria-describedby={form?.errors.context
                  ? "context-hint context-error"
                  : "context-hint"}></textarea>
              {#if form?.errors.context}<p class="field-error" id="context-error">
                  {form.errors.context}
                </p>{/if}
            </div>
            ...
            <fieldset
              class="tone-field"
              aria-describedby={form?.errors.tone ? "tone-error" : undefined}
            >
                ...
                {#each Object.entries(toneLabels) as [tone, label] (tone)}
                  <label class="tone-option">
                    <input
                      ...
                      name="tone"
                      value={tone}
                      checked={(form?.values.tone ?? "friendly") === tone}
                    />
                    <span>{label}</span>
                  </label>
                {/each}
              </div>
              {#if form?.errors.tone}<p class="field-error" id="tone-error">
                  {form.errors.tone}
                </p>{/if}
            </fieldset>
          ...
          <p class="privacy-note">
            Submitted text is sent to OpenAI. Leave out passwords and sensitive customer details.
          </p>
        </form>
      </section>
      ...
      <section class="panel draft-panel" aria-labelledby="draft-heading" aria-busy={pending}>
        ...
        <noscript
          ><p class="hint">
            Select the draft text to copy it. The copy button needs JavaScript.
          </p></noscript
        >
      </section>
    </div>
